...
export class ActFile extends Model {
  ...
  public getSignification(): Promise<Signification> {
    if (this._user) return Promise.resolve(this._signification);
    if (!this.significationId) return Promise.reject(new Error("Can't get signification because userId si undefined"));
    if (!this.significationId) return Promise.reject(new Error("Can't get signification because actId si undefined"));
